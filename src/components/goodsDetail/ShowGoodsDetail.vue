    ...
    <div class="item-intro-show">
        ...
        <div class="item-intro-recommend-column">
          <div class="item-recommend-column" v-for="(item, index) in goodsInfo.hot" :key="index">
            ...
            <div class="item-recommend-intro">
              <span>
                <span class="item-recommend-top-num">{{index + 1}}</span> {{ $t('showGoodsDetail.hot') }}{{item.sale}}{{ $t('showGoodsDetail.piece') }}</span>
      ...
      <div class="item-intro-detail" ref="itemIntroDetail">
        <div class="item-intro-nav item-tabs">
          <Tabs>
            <TabPane :label="$t('showGoodsDetail.introTab')">
              <div class="remarks-title">
                <span>{{ $t('showGoodsDetail.introTab') }}</span>
              </div>
              <div class="item-intro-img" ref="itemIntroGoods">
                <img :src="item" alt="" v-for="(item,index) in goodsInfo.goodsDetail" :key="index">
              </div>
            </TabPane>
            <TabPane :label="$t('showGoodsDetail.paramTab')">
              <div class="remarks-title">
                <span>{{ $t('showGoodsDetail.paramTab') }}</span>
              </div>
              <div class="item-param-container">
                <span class="item-param-box" v-for="(item,index) in goodsInfo.param" :key="index">
                  <span class="item-param-title">{{ $t(item.title) }}: </span>
                  <span class="item-param-content">{{item.content}}</span>
                </span>
              </div>
            </TabPane>
            <TabPane :label="$t('showGoodsDetail.warrantyTab')">
              <ShowProductWarranty></ShowProductWarranty>
            </TabPane>
            <TabPane :label="$t('showGoodsDetail.remarksTab')">
              <div class="remarks-container">
                <div class="remarks-title">
                  <span>{{ $t('showGoodsDetail.remarksTab') }}</span>
                </div>
                <div class="remarks-analyse-box">
                  <div class="remarks-analyse-goods">
                    <i-circle :percent="goodsInfo.remarks.goodAnalyse" stroke-color="#e4393c">
                      ...
                      <p class="remarks-analyse-title">{{ $t('showGoodsDetail.goodRate') }}</p>
                    </i-circle>
                  </div>
                ...
                <div class="remarks-bar">
                  <span>{{ $t('showGoodsDetail.append') }}({{goodsInfo.remarks.remarksNumDetail[0]}})</span>
                  <span>{{ $t('showGoodsDetail.good') }}({{goodsInfo.remarks.remarksNumDetail[1]}})</span>
                  <span>{{ $t('showGoodsDetail.normal') }}({{goodsInfo.remarks.remarksNumDetail[2]}})</span>
                  <span>{{ $t('showGoodsDetail.bad') }}({{goodsInfo.remarks.remarksNumDetail[3]}})</span>
                </div>
